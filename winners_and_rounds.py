...
def winners(ls, ls2, i):
    ...
    # if index 11 is empty, this means a player won in two games (i.e. she won 2-0), and so we check the score
    # in both indices (9 and 10) and see if a player had a higher number in both of them, she won. Otherwise, the
    # other player won:
...
# The next function was used to determine the number of tournaments per format (format = number of games in the tournament)
...
def rounds_knockout(ls2, num_of_games, line):
    ...
    # From the quarter finals we add the names and numbers of the rounds manually
    for k in range(1, 5):
        ls2[line][4] = 'QF'
...
# Writing the main function that will call all the rounds functions, and will determine the round for each game
...
# some of the files. We will fetch them and hardcode them:
...
# Writing a function to count the number of games per tournament to determine what kind of format it is:
...
def final_winner_year(ls, tournament, year):
    """Takes three arguments - the list (dataset), name of tournament and year. Prints the winner
