...
# For question 3 we write a function that will run on all rounds as we defined in question 1 (the last index of each line -
...
def wbw_rank(ls, year, year_2):
    ...
    # Creating a similar table, but without the index of the first player (so only the indices of the winners of each player)
    ...
    # which is the same score in the beginning for each player (1/the total number of players), and the second list with 0
    # as the initial score for each player:
    ...
    # Once the delta (the absoult difference between the scores in the first_table and second_table) is smaller than the threshold,
    # we can say that the algorithm has converged and stop the iterations
